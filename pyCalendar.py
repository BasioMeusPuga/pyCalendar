...
def calendar_add():
	...
	try:
		...
		exit()
	event_repeat = input('Repeat? (YES/no) ')
	if event_repeat != 'no':
		event_repeat = 'yes'

	event_insert = input('Insert into Database? (yes/NO) ')
	if event_insert == 'yes':
...
def parse_ics(ics_file):
	...
	for i in _calendar:
		...
		# and optionally, the UNTIL value hasn't been exceeded by the
		# time the next event occurs
